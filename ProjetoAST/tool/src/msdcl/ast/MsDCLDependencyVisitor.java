...
public class MsDCLDependencyVisitor extends ASTVisitor{
				...
				for (Object it : node.superInterfaceTypes()) {
					...
					case ASTNode.SIMPLE_TYPE:
						...
						if (t.getFullyQualifiedName().equals(st.getName().resolveTypeBinding().getQualifiedName())) {
							if (!type.isInterface()) {
								this.dependencies.add(new ImplementsDependency(this.className, t.getFullyQualifiedName(),
										fullClass.getLineNumber(st.getStartPosition()), st.getStartPosition(), st.getLength()));
							} else {
								this.dependencies.add(new ExtendsDirectDependency(this.className, t.getFullyQualifiedName(), fullClass
	...
	@Override
	public boolean visit(SingleMemberAnnotation node) { // Visita à uma anotação definida em uma classe. No contexto do MsDCL será considerada as anotações @Autowired (Spring)
		if (node.getParent().getNodeType() == ASTNode.FIELD_DECLARATION) {
			FieldDeclaration field = (FieldDeclaration) node.getParent();
			this.dependencies.add(new FieldAnnotationDependency(this.className, node.getTypeName().resolveTypeBinding().getQualifiedName(),
			...
			this.dependencies.add(new MethodAnnotationDependency(this.className, node.getTypeName().resolveTypeBinding().getQualifiedName(),
			...
			this.dependencies.add(new ClassAnnotationDependency(this.className, node.getTypeName().resolveTypeBinding().getQualifiedName(),
					fullClass.getLineNumber(node.getStartPosition()), node.getStartPosition(), node.getLength()));
		}
		return true;
	}
